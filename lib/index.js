...
function babelPluginMithrilComponentDataAttrs({
  ...
  function nameForAnonCallExpression(path, file) {
    // try to get a named function parent
    let p = path;

    while (p) {
      var _p$node, _p$node$id;

      p = p.getFunctionParent();

      if (t.isFunctionDeclaration(p) && p.isFunctionDeclaration() && (_p$node = p.node) !== null && _p$node !== void 0 && (_p$node$id = _p$node.id) !== null && _p$node$id !== void 0 && _p$node$id.name) {
  ...
  const mithrilAttrVisitor = {
    ...
    Identifier: (path, state) => {
      ...
      path; // this is so you can use 'attrs' convention when Identifier is unknown at compile-time

      if (path.node.name === "attrs") {
        state.target.replaceWith(merge(t.objectExpression([createObjectProperties(state.name)]), state.memeberParent || path.node));
      }
  ...
  const addAttribute = (path, state) => {
    ...
    if (t.isObjectExpression(target) || t.isMemberExpression(target) || t.isIdentifier(target) || t.isCallExpression(target)) {
      path.traverse(mithrilAttrVisitor, {
        target: targetPath,
        ...state
      });
      return;
  ...
  const merge = (obj, expr) => {
    return t.expressionStatement(t.callExpression(t.memberExpression(t.identifier("Object"), t.identifier("assign")), [obj, expr]));
  };
